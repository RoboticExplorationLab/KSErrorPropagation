#!/usr/bin/env julia
"""
Julia script to run propagate_ks_keplerian_dynamics and save results to CSV.
...
include(joinpath(julia_project, "src", "ks_dynamics.jl"))
...
if length(ARGS) != 5
    error("Usage: julia run_julia_ks_numerical.jl <a> <e> <mu> <times_file> <output_file>")
end
...
h_0 = energy_ks(ks_state_0[1:4], ks_state_0[5:8], mu)
