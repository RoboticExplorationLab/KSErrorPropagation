"""
Compare Python and Julia KS numerical solutions from saved data files.
"""
...
def compare_results(python_file, julia_file, output_file, a, e, mu):
    ...
    # Load Julia CSV
    julia_data = np.loadtxt(julia_file, delimiter=",", skiprows=1)
    ...
    comparison = {
        ...
        "integrators": {
            "python": "RK45 (Runge-Kutta 4/5)",
            "julia": "Tsit5 (Tsitouras 5/4)",
            "note": "Different but similar adaptive Runge-Kutta methods",
        },
        "tolerances": {
            "atol": 1e-12,
            "rtol": 1e-13,
    ...
    print(f"\nIntegrators:")
    print(f"  Python: {comparison['integrators']['python']}")
    print(f"  Julia: {comparison['integrators']['julia']}")
    print(f"  Note: {comparison['integrators']['note']}")
    print(f"\nTolerances:")
    print(f"  atol = {comparison['tolerances']['atol']}")
    print(f"  rtol = {comparison['tolerances']['rtol']}")
    print(f"\nPosition Errors:")
...
def main():
    ...
    print("=" * 80)
    print("COMPARING PYTHON AND JULIA KS NUMERICAL SOLUTIONS")
    print("=" * 80)
    ...
    for test_case in test_cases:
        ...
        python_file = os.path.join(data_dir, f"results_python_ks_numerical_{test_case['name']}.csv")
        julia_file = os.path.join(data_dir, f"results_julia_ks_numerical_{test_case['name']}.csv")
        comparison_file = os.path.join(data_dir, f"comparison_ks_numerical_{test_case['name']}.json")

        # Check if files exist
        if not os.path.exists(python_file):
            print(f"  Warning: Python results file not found: {python_file}")
            print("  Run run_python_ks_numerical.py first")
            continue

        if not os.path.exists(julia_file):
            print(f"  Warning: Julia results file not found: {julia_file}")
            print("  Run run_julia_ks_numerical.py first")
            continue
