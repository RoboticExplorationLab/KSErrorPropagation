...
# Orbit parameters
sma = 750e3 + R_EARTH  # semi-major axis
e = 0.0  # eccentricity
i = deg2rad(98.2)  # inclination
ω = deg2rad(0.0)  # argument of periapsis
Ω = deg2rad(0.0)  # right angle of the ascending node
M = deg2rad(0.0)  # Mean Anomaly
...
println("  Argument of periapsis: ", ω * 180 / π, " deg")
println("  RAAN: ", Ω * 180 / π, " deg")
...
# Initial state covariance (diagonal: 1m position, 0.01 m/s velocity)
σ_pos = 1.0  # meters
σ_vel = 0.01  # m/s
...
for i in 1:N
    ...
    if pos_error_sp > max_mean_pos_error_sp
        global max_mean_pos_error_sp = pos_error_sp
    ...
    if vel_error_sp > max_mean_vel_error_sp
        global max_mean_vel_error_sp = vel_error_sp
    end
    ...
    if pos_error_lin_ks > max_mean_pos_error_lin_ks
        global max_mean_pos_error_lin_ks = pos_error_lin_ks
    end
    if vel_error_lin_ks > max_mean_vel_error_lin_ks
        global max_mean_vel_error_lin_ks = vel_error_lin_ks
    end
    ...
    if pos_error_lin_cart > max_mean_pos_error_lin_cart
        global max_mean_pos_error_lin_cart = pos_error_lin_cart
    end
    if vel_error_lin_cart > max_mean_vel_error_lin_cart
        global max_mean_vel_error_lin_cart = vel_error_lin_cart
    end
end
...
for i in 1:N
    ...
    if cov_error_sp > max_cov_error_sp
        global max_cov_error_sp = cov_error_sp
    end
    ...
    if cov_error_lin_ks > max_cov_error_lin_ks
        global max_cov_error_lin_ks = cov_error_lin_ks
    end
    ...
    if cov_error_lin_cart > max_cov_error_lin_cart
        global max_cov_error_lin_cart = cov_error_lin_cart
    end
end
...
savefig(p_combined, "figs/test_error_propagation_comparison.png")
println("  Saved plot to figs/test_error_propagation_comparison.png")
