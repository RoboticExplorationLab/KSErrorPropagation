"""
Test script to verify that KS relative dynamics match full nonlinear propagation.
Compares propagate_ks_keplerian_relative_dynamics against separate propagation
of chief and deputy in Cartesian coordinates.
"""
...
sma_c = 750e3 + R_EARTH  # semi-major axis
...
println("="^80)
println("KS ERROR PROPAGATION - RELATIVE DYNAMICS MATCHING TEST")
println("="^80)
println("Testing KS relative dynamics against full nonlinear propagation")
println("\nChief orbit:")
...
x_vec_chief_0 = SD.sOSCtoCART(oe_vec_chief; GM=GM, use_degrees=false)
r_vec_chief_0 = x_vec_chief_0[1:3]
v_vec_chief_0 = x_vec_chief_0[4:6]

oe_vec_deputy = [sma_d, e_d, i_d, Ω_d, ω_d, M_d]
x_vec_deputy_0 = SD.sOSCtoCART(oe_vec_deputy; GM=GM, use_degrees=false)
...
# Full nonlinear propagation: propagate chief and deputy separately
println("\nPropagating chief in Cartesian coordinates...")
x_vec_traj_chief_cart, t_traj_chief_cart = propagate_cartesian_keplerian_dynamics(x_vec_chief_0, times, SIM_PARAMS, GM)
println("  Completed: ", length(x_vec_traj_chief_cart), " states")

println("Propagating deputy in Cartesian coordinates...")
x_vec_traj_deputy_cart, t_traj_deputy_cart = propagate_cartesian_keplerian_dynamics(x_vec_deputy_0, times, SIM_PARAMS, GM)
...
# KS relative dynamics propagation
println("Propagating using KS relative dynamics...")
x_vec_traj_chief_ks, x_vec_traj_rel_ks, t_chief_traj, t_deputy_traj = propagate_ks_keplerian_relative_dynamics(
...
println("\nMaximum errors (Relative state: Full nonlinear vs KS relative dynamics):")
...
p1 = plot(title="Chief Position Comparison", xlabel="Time (h)", ylabel=chief_pos_ylabel)
...
p2 = plot(title="Chief Position Errors", xlabel="Time (h)", ylabel=chief_pos_error_ylabel)
...
p3 = plot(title="Relative Position Comparison", xlabel="Time (h)", ylabel=rel_pos_ylabel)
...
p4 = plot(title="Relative Position Errors", xlabel="Time (h)", ylabel=rel_pos_error_ylabel)
...
p5 = plot(title="Relative Velocity Errors", xlabel="Time (h)", ylabel=rel_vel_error_ylabel)
plot!(p5, times[1:N] / 3600, vel_errors_rel / rel_vel_error_scale,
    label="Full nonlinear - KS relative", linewidth=2, color=:red)

p_combined = plot(p1, p2, p3, p4, p5, layout=(5, 1), size=(800, 2000))
savefig(p_combined, "figs/test_keplerian_relative_dynamics_match.png")
println("  Saved plot to figs/test_keplerian_relative_dynamics_match.png")
