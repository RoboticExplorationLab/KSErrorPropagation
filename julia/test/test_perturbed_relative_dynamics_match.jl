"""
Test script to verify that KS perturbed relative dynamics match full nonlinear propagation.
Compares propagate_ks_perturbed_relative_dynamics against separate propagation
of chief and deputy in Cartesian coordinates with perturbations (J2 + drag).
"""
...
const epoch_0 = Epoch(2000, 1, 1, 12, 0, 0)  # initial epoch

# Define simulation parameters
const SIM_PARAMS = (
    # Number of orbits to simulate
    num_orbits=1.0,  # Start with 1 orbit for faster testing
    ...
    max_pos_error_threshold=10.0,      # meters (relaxed for perturbed case)
    max_vel_error_threshold=0.1,       # m/s (relaxed for perturbed case)
)
...
println("  J2 coefficient: ", J2)
println("  Earth rotation rate: ", OMEGA_EARTH, " rad/s")
println("  Drag coefficient: ", CD)
println("  Cross-sectional area: ", A, " m²")
println("  Mass: ", m, " kg")
println("  Initial epoch: ", epoch_0)
...
println("\nPropagating chief in Cartesian coordinates (with perturbations)...")
x_vec_traj_chief_cart, t_traj_chief_cart = propagate_cartesian_perturbed_dynamics(
    x_vec_chief_0, times, SIM_PARAMS, GM, R_EARTH, J2, epoch_0, OMEGA_EARTH, CD, A, m)
println("  Completed: ", length(x_vec_traj_chief_cart), " states")

println("Propagating deputy in Cartesian coordinates (with perturbations)...")
x_vec_traj_deputy_cart, t_traj_deputy_cart = propagate_cartesian_perturbed_dynamics(
    x_vec_deputy_0, times, SIM_PARAMS, GM, R_EARTH, J2, epoch_0, OMEGA_EARTH, CD, A, m)
...
x_vec_traj_chief_ks, x_vec_traj_rel_ks, t_chief_traj, t_deputy_traj = propagate_ks_perturbed_relative_dynamics(
    x_vec_chief_0, x_vec_deputy_0, times, SIM_PARAMS, GM, R_EARTH, J2, epoch_0, OMEGA_EARTH, CD, A, m)
...
savefig(p_combined, "figs/test_perturbed_relative_dynamics_match.png")
println("  Saved plot to figs/test_perturbed_relative_dynamics_match.png")
